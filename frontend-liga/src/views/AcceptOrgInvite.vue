    ...
    <div class="auth-card card w-full">
      ...
      <template v-else-if="state === 'success'">
        <div class="text-center">
          ...
          <p v-if="isNew">
            Tu cuenta ha sido creada. Ya puedes iniciar sesión como Administrador de la liga <strong>{{ orgName }}</strong>.
          </p>
          <p v-else>
            Tu acceso como Administrador de la liga <strong>{{ orgName }}</strong> ya está activo. Inicia sesión para continuar.
          </p>
          <router-link to="/login" class="btn btn-primary btn-full mt-md">
            Iniciar sesión
          </router-link>
      ...
      <template v-else-if="state === 'existing'">
        <div class="text-center">
          ...
          <h2 class="mb-sm">Invitación a {{ orgName }}</h2>
          <p>Ya tienes una cuenta asociada a <strong>{{ emailMasked }}</strong>. Tu acceso como Administrador de la liga ya fue activado.</p>
      ...
      <template v-else-if="state === 'form'">
        ...
        <div class="text-center mb-lg">
          ...
          <p>Has sido invitado como Administrador de la liga <strong>{{ orgName }}</strong>.<br>Crea una contraseña para activar tu cuenta.</p>
        </div>
...
onMounted(async () => {
  ...
  try {
    const res = await apiClient.get('/auth/org-invite-info', { params: { token: t } })
...
const confirmExisting = async () => {
  ...
  error.value = ''
  try {
    await apiClient.post('/auth/accept-org-invite', {
      token:    token.value,
