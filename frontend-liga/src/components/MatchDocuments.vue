<template>
  <div class="card">
    <div class="flex justify-between items-center mb-md">
      <h3 class="section-title mb-0">Documentación del Partido</h3>
    ...
    <div v-if="!loading && documents.length === 0" class="text-center py-lg text-muted text-sm">
      No hay documentos adjuntos para este partido.
    </div>
      ...
      <div class="modal-box">
        ...
        <div v-if="selectedFile" class="mt-md">
          ...
          <input
            ...
            placeholder="Ej: Acta de partido, Comprobante..."
            :disabled="uploading"
            @click.stop
          />
        </div>
...
import {
  ...
  deleteDocument,
  formatFileSize,
} from '../services/match_documents.service.js';
...
async function loadDocuments() {
  ...
  listError.value = '';
  try {
    const res = await listDocuments(props.matchId);
...
async function doDelete() {
  ...
  deleting.value = true;
  try {
    await deleteDocument(props.matchId, docToDelete.value.id);
